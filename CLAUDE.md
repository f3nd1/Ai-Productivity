...
- **Quick Fill** (`tabs/QuickFill.jsx` + pure `quickfill.js`) turns one rough pasted note into B8/B9/B10 plus proposed Section C results via `POST /api/quick-fill` (JSON mode, analysis model). It is paste → **review** → apply: nothing touches the page until Apply, Apply only populates in-memory state (the page Save still persists it), and replacing existing B text always warns first. `quickfill.js` is where "never invent" is enforced structurally — unusable proposals are dropped, blanks stay `null` rather than becoming `0` or `''`, and financial results get **no** fabricated money figure since a rough note rarely states the saving basis. Blur autosave is suppressed while the modal is open.
