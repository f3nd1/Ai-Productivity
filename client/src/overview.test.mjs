...
assert.equal(a.completeness, 6); // b8, b9, c11 + the 3 shared D answers
...
assert.equal(b.completeness, 4); // b8 + the same 3 shared D answers
...
assert.equal(bare.rows.find((r) => r.id === 'B').completeness, 1); // b8 only
